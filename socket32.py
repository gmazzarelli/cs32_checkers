...
class Socket32:
    ...
    def send_move(self, piece: Tuple[int, int], move_lst: List[Tuple[int, int]]):
        """Send a move over the socket.

        Args:
            Tuple: the coordinate of the piece moved
            List[Tuple]: a list of coordinates of each move
        
        """
        msg = json.dumps({'piece': piece, 'move_lst': move_lst})
        msg_bytes = msg.encode('utf-8')
        self._sock.sendall(msg_bytes)

    def recv_move(self) -> Tuple[Tuple[int, int], List[Tuple[int, int]]] | None:
        """Receive a move from the socket.
        
        Returns:
            Tuple: the coordinate of the opponent's piece moved
            List[Tuple]: a list of coordinates of each move by the opponent
        """
        msg_bytes = self._sock.recv(BUFFER_SIZE)
        data_decoded = msg_bytes.decode('utf-8')
        if not data_decoded:
            return None

        json_data = json.loads(data_decoded)
        piece = tuple(json_data['piece'])
        move_lst = [tuple(coord) for coord in json_data['move_lst']]
        return piece, move_lst
